...
import {
  ...
  CommandItem,
  CommandList,
} from "@tbms/ui/components/command";
import {
  Popover,
  PopoverContent,
  PopoverTrigger,
} from "@tbms/ui/components/popover";
      ...
      <PopoverTrigger
        render={
          <Button
            ...
            aria-expanded={open}
            className={cn(
              "h-auto min-h-11 w-full justify-between border-border bg-background px-3 py-2 transition-all hover:border-border hover:bg-card",
              className,
            )}
          />
        }
            ...
            selected.map((value) => {
              ...
              return (
                <Badge
                  key={value}
                  variant="default"
                  className="rounded-md border-primary/10 bg-muted pr-1 text-primary transition-colors hover:bg-accent"
                  onClick={(e) => {
                    e.stopPropagation();
