...
async function main() {
  ...
  for (const item of itemsToSplit) {
    ...
    console.log(
      `Splitting Item ID: ${item.id}, Qty: ${qty}, Order: ${item.orderId}`,
    );
  ...
  for (const order of orders) {
    const garmentSequenceMap: Record<string, number> = {};
    for (const item of order.items) {
      garmentSequenceMap[item.garmentTypeId] =
        (garmentSequenceMap[item.garmentTypeId] || 0) + 1;
      if (item.pieceNo !== garmentSequenceMap[item.garmentTypeId]) {
        await prisma.orderItem.update({
          where: { id: item.id },
          data: { pieceNo: garmentSequenceMap[item.garmentTypeId] },
        });
        console.log(
          `Updated Order ${order.orderNumber} Item ${item.id} pieceNo to ${garmentSequenceMap[item.garmentTypeId]}`,
        );
      }
    }
  }
